...
arma::mat COptimiserVMDFP::UpdateHessian ()
{
	double denom1 = (m_deltaX.t ()*m_deltaGrad).eval ()[0];
	assert (std::abs (denom1) > m_rTolerance);
	double denom2 = (m_deltaGrad.t ()*m_mHess*m_deltaGrad).eval ()[0];
	assert (std::abs (denom2) > m_rTolerance);

	return m_deltaX*(m_deltaX.t ()) / denom1 - (m_mHess*m_deltaGrad)*(m_mHess*m_deltaGrad).t () / denom2;
}

//////////////////////////////////////////////////

double COptimiserVMDFP::GetMinimalScaleValue (std::shared_ptr<CLinearFunction> ptrVF, CMinimiserGolden& minimiser, const arma::vec& direction)
{
...
void COptimiserVMDFP::Optimise (double target)
{
